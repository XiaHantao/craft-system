...
import { listNonStructuralTooling2, getNonStructuralTooling, delNonStructuralTooling, addNonStructuralTooling, updateNonStructuralTooling } from "@/api/ToolingModule/NonStructuralTooling";
...
function getList() {
  loading.value = true;
  listNonStructuralTooling2(queryParams.value).then(response => {
  ...
  proxy.$refs["NonStructuralToolingRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        ...
        form.value.moldOwnership = 'materialPreparationTeamTooling';
