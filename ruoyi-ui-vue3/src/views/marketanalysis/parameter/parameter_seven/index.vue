<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="Plus" @click="handleAdd"
          v-hasPermi="['marketanalysis:parameter_seven:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate"
          v-hasPermi="['marketanalysis:parameter_seven:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['marketanalysis:parameter_seven:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="parameter_sevenList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="parameter_sevenRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="制造商" prop="manufacturer">
...
function getList() {
  ...
  listParameter_seven(queryParams.value).then(response => {
    parameter_sevenList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("parameter_sevenRef");
}
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["parameter_sevenRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateParameter_seven(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addParameter_seven(form.value).then(response => {
...
function handleExport() {
  proxy.download('marketanalysis/parameter_seven/export', {
    ...queryParams.value
  }, `parameter_seven_${new Date().getTime()}.xlsx`)
}
