...
<script setup name="Parameterseven">
import { listParameterseven, getParameterseven, delParameterseven, addParameterseven, updateParameterseven } from "@/api/marketanalysis/parameter/parameter_seven";

const { proxy } = getCurrentInstance();

const parametersevenList = ref([]);
...
function getList() {
  loading.value = true;
  listParameterseven(queryParams.value).then(response => {
    parametersevenList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("parametersevenRef");
}
...
function handleUpdate(row) {
  ...
  getParameterseven(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["parametersevenRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateParameterseven(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addParameterseven(form.value).then(response => {
...
function handleExport() {
  proxy.download('marketanalysis/parameterseven/export', {
    ...queryParams.value
  }, `parameterseven_${new Date().getTime()}.xlsx`)
}
