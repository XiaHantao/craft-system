...
function getList() {
  ...
  listParameter_two(queryParams.value).then(response => {
    parameter_twoList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("parameter_twoRef");
}
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["parameter_twoRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateParameter_two(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addParameter_two(form.value).then(response => {
